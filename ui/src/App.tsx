    ...
    <div className="flex flex-col h-full gap-5 xl:flex-row ">
      <main className="flex flex-col grow gap-5">
        <nav className={`flex flex-wrap items-stretch justify-between grow rounded-[10px] bg-demo ${shouldOpen ? 'h-auto' : 'max-h-[89px]'} w-full`}>
          ...
          <div className="hidden lg:flex lg:justify-evenly lg:mr-5 text-3xl text-[var(--color)] w-32 xl:w-3xs font-medium items-center">
            <LuBell />
            <LuSettings />
            <LuMessageCircle />
          </div>
          <div className={`justify-between items-end w-full ${shouldOpen ? '' : 'hidden'}`}>
            <Search searchValue={searchValue} onSearchChange={onChangeSearch} />
            <div className="flex-col flex mt-4 text-[var(--color)]">
              {isLoading && (
                <div className="flex gap-2">
                  {[...Array(4)].map((_, i) => (
                    <div
              ...
              {!isLoading && categories.length && categories.map(chip => (
                <button
                  key={chip}
                  onClick={() => changeFilter(chip)}
                  className={`capitalize cursor-pointer ${filterSelected === chip ? 'bg-[var(--lyric-green)]' : 'bg-[var(--bg-color)] hover:text-gray-600 hover:font-semibold hover:bg-gray-50'} block py-2 px-3 min-w-20 rounded-xl`}
                >{chip}</button>
              ))}
            </div>
          </div>
      ...
      <aside className={`w-full xl:relative ${resizeLayout ? 'hidden' : 'xl:w-[20%] justify-between'} rounded-[10px] bg-demo px-[30px] py-10`}>
        ...
        <div className="flex flex-wrap w-full bg-[var(--bg-color)] rounded-[10px] py-5 px-2.5">
